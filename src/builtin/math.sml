...
let
  fun loop ss = fn
    []             => Expr.eps
  | [e]            => Expr.ieval (Dict.fold (fn k => fn v => fn E' => Environment.upLocal E' k (Formula v)) ss E) e
  | e1 :: e2 :: es =>
  let
    val t1 = Expr.getFormula (Expr.ieval E e1)
    val t2 = Expr.getFormula (Expr.ieval E e2)
  in
